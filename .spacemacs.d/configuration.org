...
#+begin_src emacs-lisp :results none
  ...
  (spacemacs/set-leader-keys "og" 'jm/open-with-gogland)
...
#+begin_src emacs-lisp :results none
  ...
  (defun jm/github-open-pr ()
    ...
    (let ((branch (jm/git-current-branch))
      ...
      (jm/open (if (string= branch "master")
                   repo
                 (concat repo "/compare/master..." branch)))))
...
#+begin_src emacs-lisp :results none
  ...
  (defun jm/open-with-gogland ()
    (interactive)
    (jm/open-with-reveal "Gogland")
    (jm/open-with-line "/usr/local/bin/gogland"))
